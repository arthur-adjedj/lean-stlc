...
modular BoolTerm
  ...
  @[simp, grind]
  def Term.is_bool_lit : Term -> Prop
    | .true | .false => True
    | _ => False
  ...
  @[grind =, simp]
  theorem subst_true {σ} : (Term.true)[σ] = Term.true := by
    rfl
  @[grind =, simp]
  theorem subst_false {σ} : (Term.false)[σ] = Term.false := by
    rfl
  @[grind =, simp]
  theorem subst_ite {σ} : (Term.ite b Pt Pf)[σ] = Term.ite b[σ] Pt[σ] Pf[σ] := by
    rfl
  ...
  mod inductive ParRed extends ParRed where
    ...
    | ite :
      ...
      ParRed Pt Pt' ->
      ParRed Pf Pf' ->
      ParRed (.ite b Pt Pf) (.ite b' Pt' Pf')
    | iteTrue :
      ParRed Pt Pt' ->
      -- ParRed PS PS' ->
      ParRed (.ite .true Pt Pf) Pt'
    | iteFalse  :
      ParRed Pf Pf' ->
      -- ParRed PS PS' ->
      ParRed (.ite .false Pt Pf) Pf'
  ...
  := by
    ...
    case app  =>
      simp only [subst_app]
      apply ParRed.app <;> grind only [= subst_true]
    case lam ih =>
      simp only [subst_lam]
      apply ParRed.lam
  ...
  mod inductive Red extends Red where
    ...
    | ite3 :
      ...
      Red (.ite b Pt Pf) (.ite b Pt Pf')
    | iteTrue :
      Red (.ite .true Pt Pf) Pt
    | iteFalse:
      Red (.ite .false Pt Pf) Pf
  ...
  mod def par_implies_seqs extends Red.par_implies_seqs where
      ...
      all_goals intros
      · constructor
      · apply LeanSubst.Star.refl
      · apply LeanSubst.Star.congr3 Term.ite <;> grind (splits := 0)
  ...
  mod def subst_arg extends _root_.Red.subst_arg where
      ...
      all_goals intros
      · simp; constructor
      · simp only [subst_false]
        exact Star.refl
      · simp only [subst_ite]
        apply Star.congr3 _ Red.ite1 Red.ite2 Red.ite3 <;> grind
  ...
  mod def typing_renaming_lift extends typing_renaming_lift where
    finally all_goals grind only

  mod def typing_weaken extends typing_weaken where
    finally
    all_goals first | simp [Ren.apply,SubstMap.smap,smap] <;> grind

  mod def typing_subst_lift extends typing_subst_lift where
    finally
      all_goals grind only

  mod def typing_subst extends typing_subst where
    finally
      all_goals grind

  mod def typing_beta extends typing_beta where
    finally
      all_goals grind only
  ...
  mod inductive Value extends Value where
    ...
    | ite : Value b → Value Pt → Value Pf →
      ¬ b.is_bool_lit → Value (.ite b Pt Pf)

  mod def value_sound extends value_sound where
    finally
  ...
  mod def SnHeadRed.red_compatible extends SnHeadRed.red_compatible where
      ...
      · intro _ _ _ _ snn ihn _ r
        ...
        · cases ihn ‹_› with
          ...
          | inr h =>
           obtain ⟨z, ih1, ih2⟩ := h
           exact .inr ⟨_, .iteStep ih1, .congr3_1 _ _ .ite .ite1 ih2⟩
        · exact .inr ⟨_,.iteStep snn, Star.step1 (Red.ite2 ‹_›)⟩
        · exact .inr ⟨_,.iteStep snn, Star.step1 (Red.ite3 ‹_›)⟩
        · cases snn
        · cases snn

  namespace SN
  theorem subterm_ite : SN Red (.ite b Pt Pf) -> SN Red b ∧ SN Red Pt ∧ SN Red Pf := by
    intro h
    generalize e : b.ite Pt Pf = t at h
    induction h generalizing b Pt Pf <;> cases e
    rename_i a a_ih
    refine ⟨?_,?_,?_⟩ <;> constructor
    · intro _ r
      exact a_ih _ (Red.ite1 r) rfl |>.1
    · intro _ r
      exact a_ih _ (Red.ite2 r) rfl |>.2.1
    · intro _ r
      exact a_ih _ (Red.ite3 r) rfl |>.2.2
  ...
  theorem neutral_ite {b Pt Pf} :
    ...
    SN Red Pt ->
    SN Red Pf ->
    SN Red (Term.ite b Pt Pf)
  := by
    intro nh j1 j2 j3
    induction j1 generalizing Pt Pf; case _ b h1 ih1 =>
    induction j2 generalizing Pf; case _ Pt h2 ih2 =>
    induction j3 ; case _ Pf h3 ih3 =>
    apply SN.sn; case _ =>
    intro y r; cases r
    case iteTrue => cases nh
    case iteFalse => cases nh
    case ite1 b' r =>
      apply ih1 _ r _ (.sn h2) (.sn h3)
      apply Red.preservation_of_neutral_step nh r
    case ite2 Pt' r => apply ih2 _ r (.sn h3)
    case ite3 Pf' r => apply ih3 _ r
  ...
  := by
    ...
    induction h1 generalizing b' Pt Pf; case _ b hb ihb =>
    induction h2 generalizing Pf; case _ Pt hPt ihPt =>
    induction h3; case _ Pf hPf ihPf =>
    ...
    case ite1 b'' r =>
      ...
      case _ lem1 =>
        ...
        apply ihb b'' r lem1 (SN.sn hPt) (SN.sn hPf)
        apply SN.preservation h4
        exact Star.congr3_1 _ _ _ Red.ite1 lem2
    case ite2 Pt' r =>
      apply ihPt Pt' r
      · constructor
        assumption
  ...
  theorem true_expansion : SN Red s -> SN Red z -> SN Red (.ite .true z s) := by
    intro h1 h2
    induction h2 generalizing s; case _ z hz ihz =>
    induction h1; case _ s hs ihs =>
    apply SN.sn; case _ =>
    intro y r; cases r
    case iteTrue => apply SN.sn hz
    case ite1 z' r => cases r
    case ite2 s' r =>
      apply ihz _ r
      apply SN.sn hs
    case ite3 n' r => apply ihs _ r

  theorem false_expansion :
    SN Red z ->
    SN Red s ->
    SN Red (.ite .false z s)
  := by
    intro h1 h2
    induction h2 generalizing z; case _ z hz ihz =>
    induction h1; case _ s hs ihs =>
    apply SN.sn; case _ =>
    intro y r; cases r
    case iteFalse => apply SN.sn hz
    case ite1 z' r => cases r
    case ite3 s' r =>
      apply ihz _ r
      apply SN.sn hs
    case ite2 n' r => apply ihs _ r
  ...
  theorem backward_closure {t' t} : SN Red t' -> SnHeadRed t t' -> SN Red t := by
    ...
    case iteTrue  h2 => apply true_expansion h2 h
    case iteFalse h2 => apply false_expansion h2 h
    case iteStep r ih =>
      obtain ⟨h1, h2, h3⟩ := subterm_ite h
      apply backward_closure_ite r (ih h1) h2 h3 h
  end SN
  ...
  mod inductive SNi extends SNi where
    ...
    | iteTrue : SNi .nor Pf → SNi .red (.ite .true Pt Pf, Pt)
    | iteFalse : SNi .nor Pt → SNi .red (.ite .false Pt Pf, Pf)
    | iteStep : SNi .red (b, b') → SNi .red (.ite b Pt Pf, .ite b' Pt Pf)
  ...
  mod def rename extends SNi.rename where
      ...
      · exact SNi.true
      · exact SNi.false
      · simp only [SNi.SnRenameLemmaType,subst_ite] at *
        constructor <;> grind only
      · rw [SNi.SnRenameLemmaType,subst_ite]
        constructor
        rename_i ih _
        apply ih
      · rw [SNi.SnRenameLemmaType,subst_ite]
        ...
        rename_i ih _
        apply ih
      · rw [SNi.SnRenameLemmaType,subst_ite,subst_ite]
        constructor
        rename_i ih _
        apply ih
  ...
  mod def antirename extends SNi.antirename where
    ...
    · rename_i P0_ih PS_ih _ z e
      cases z <;> simp only [subst_var,subst_app,subst_lam,subst_true, subst_false,subst_ite] at e <;> try cases e
      rename_i n _ _
      cases n <;> simp only [subst_var,subst_app,subst_lam,subst_true, subst_false,subst_ite] at e <;> cases e
      refine ⟨_, rfl, ?_⟩
      apply iteTrue
      apply PS_ih _ _ rfl
    ...
    · rename_i snn n_ih r z e
      cases z <;> simp only [subst_var,subst_app,subst_lam,subst_true, subst_false,subst_ite] at e <;> cases e
      rename_i n P0 PS k
      have ⟨w,e,_⟩ := n_ih r _ rfl
      cases e
      refine ⟨.ite w PS k, by simp, ?_⟩
      apply iteStep
      assumption
  ...
  mod def sound extends SNi.sound where
    ...
    · constructor
      ...
      cases r
    · intros
      apply SN.neutral_ite <;> try assumption
...
| true => true
| false => false
| ite z s n => ite (z.rename m) (s.rename m) (n.rename m)
...
| true => true
| false => false
| ite z s n => ite (z.subst m) (s.subst m) (n.subst m)
...
def LR (Γ : List Ty) : Ty -> Term -> Prop
  ...
  | .bool => λ t => Γ ⊢ t : Ty.bool ∧ SNi .nor t
...
theorem LR.ite_neutral
    ...
    : LR Γ A (.ite n z s)
  :=
    let lem := Typing.ite h3 (LR.typing h1) (LR.typing h2)
  ...
  theorem LR.ite (h1 : LR Γ A z) (h2 : LR Γ A s) (j : Γ ⊢ n : Ty.bool) (t : SNi .nor n)
    : LR Γ A (.ite n z s) := ite' h1 h2 t rfl j
  ...
  | @Typing.lam _ A B t tj, σ, Δ, h =>
    ...
    let lem2 {r Δ' v} (m2 : Δ -⟨r⟩> Δ') (lv : LR Δ' A v) : LR Δ' B ((:λ[A] t)[σ][r].app v) :=
      ...
      @cr.2.2 _ _ (t[.su v::σ ∘ r.to])
        ...
        tj'
    ⟨lem1, lem2⟩
  | .true, σ, Δ, h => ⟨.true, .true⟩
  | .false, σ, Δ, h => ⟨.false, .false⟩
  | .ite nj zj sj, σ, Δ, h =>
    let ⟨ih1, ih2⟩ := fundamental nj σ Δ h
    LR.ite (fundamental zj σ Δ h) (fundamental sj σ Δ h) ih1 ih2
